...
class IsAppointmentDoctor(BasePermission):
    def has_permission(self, request, view):
        ...
        hospital_uuid = view.kwargs.get("hospital_uuid")
        booking_uuid = view.kwargs.get("appointment_uuid")
        print(hospital_uuid, booking_uuid)
        hospital = Hospital.objects.filter(uuid=hospital_uuid).first()
        if not hospital:
            raise ValidationError("Wrong hospital uuid!!!")
        print(hospital)
